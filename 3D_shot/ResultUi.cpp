...
void ResultUi::ScoreDraw(int scoreFont, int countFont, int score, int excellentCount, int greatCount, int goodCount, int missCount)
{
	...
	if (displayCount > 70)
	{
		DrawFormatStringToHandle(650, 400, GetColor(255, 255, 0), countFont, "Excellent   ×  %d", excellentCount);
	}
	if (displayCount > 110)
	{
		DrawFormatStringToHandle(650, 500, GetColor(255, 105, 180), countFont, "Great         ×  %d", greatCount);
	}
	if (displayCount > 150)
	{
		DrawFormatStringToHandle(650, 600, GetColor(175, 238, 238), countFont, "Good          ×  %d", goodCount);
	}
	if (displayCount > 190)
	{
		DrawFormatStringToHandle(650, 700, GetColor(169, 169, 169), countFont, "Miss          ×  %d", missCount);
		displayCount = 190;
	}
}
